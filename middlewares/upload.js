...
const AppDetection = require("../modules/apk_detection/model/index");
...
let upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    // 打印会出现在上传文件的顶部 
    console.log('进入筛选的file', file.mimetype);
...
exports.singleUpload = function (req, res, type = '') {
  singleUpload(req, res, async function (err) {
    ...
    if (err instanceof multer.MulterError) {
      ...
      let apkIcon = apkInfo.icon
      let version = apkInfo.version;
      let user = req.session.user;
      let u_id = user.userId || "";
      let md5 = uuid.v1();  // 生成唯一的userId
      let obj = { apkIcon,ossFile, md5, u_id, apkSize, updateTime, version, apkName: apkInfo.name, package: apkInfo.package };
      if (type === "detectionApk") {
        const datas = await AppDetection.create(obj);
        // 数据处理 存在数据库 end
        if (datas) {
          res.status(200).send({ code: 1, msg: '文件上传成功' })
        }
      } else {
        console.log("我进来加固了",obj);
